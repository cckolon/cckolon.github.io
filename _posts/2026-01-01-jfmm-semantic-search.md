...
Under the hood, `sqlite-vec` is less sophisticated than PGVector. While PGVector gives you the option to use several different vector indices like [HNSW](https://arxiv.org/pdf/1603.09320) or [IVFFlat](https://docs.oracle.com/en/database/oracle/oracle-database/26/vecse/understand-inverted-file-flat-vector-indexes.html), `sqlite-vec` currently [only supports brute force search](https://github.com/asg017/sqlite-vec/issues/172), where the database checks the distance between the query vector and each row vector on each query. This uses native C libraries, but could still become a problem if I were scaling to millions of vectors, since the query time would scale with [$$O(\text{number of rows})$$](https://en.wikipedia.org/wiki/Time_complexity#Linear_time). Thankfully, I only planned on storing a constant number of chunks (about 20 thousand) and the query latency was comparable between both databases.
...
Since I was only generating one embedding per query, search was already decently fast on a CPU-only cloud virtual machine, but I was still heavily limited by memory. On startup, my server would load `nomic-embed-text-v1.5` into RAM for inference. Since I was running multiple FastAPI workers, each one would need to load a separate instance of the model. At 32-bit resolution, this was about 500 MB per worker on nomic alone - 2GB for 4 workers.

This would be trivial on my MacBook, but resources are much more limited in the cloud. I wanted to create the slimmest container image I could so that I could run it on a cheapo machine.

Luckily, there is a good solution for this. ML weights are usually stored as full-precision 32-bit floating point numbers. If we lower the precision of these numbers to fewer bits, we can save a lot of memory without affecting the model output very much. This is called [quantization](https://huggingface.co/docs/optimum/en/concept_guides/quantization). [Nomic has released](https://huggingface.co/nomic-ai/nomic-embed-text-v1.5-GGUF) quantizations of `nomic-embed-text-v1.5` ranging from 16 to 2 bit numbers.

Choosing the right quantization is a fascinating subject. For Nomic, a table is included which compares the mean squared error of embedding vectors for different quantizations compared to the 32-bit version. I made a graph of the table here: you can see that the log MSE scales inversely with the size of the model.
...
I ran the model with [`llama.cpp`](https://github.com/ggml-org/llama.cpp), a set of binaries which allows fast LLM inference on a wide variety of machines. One of `llama.cpp`'s best features is a locally-running http server (`llama-server`) which is more or less compliant with the OpenAI API. To run my embedding model, I could load the 8 bit quantization into `llama-server`, start it in embedding mode, and hit it from my FastAPI project. Starting the server looks like this:
...
Since `llama.cpp` is written in C and C++ with no dependencies, I didn't need to download any other libraries, and I could remove most of the heavyweight AI libraries from my Python code. This lowered my container build times and total size by about three quarters!

## Reranking

To address irrelevant results, I decided to add a [reranker](https://www.mongodb.com/resources/basics/artificial-intelligence/reranking-models): a model which evaluates each search result against the query and scores it by relevance. This lets us ensure that the top few results are actually the most relevant, and filter out false positive results that are actually irrelevant.
...
To avoid reintroducing the python ML libraries that I had just eliminated, I used `llama.cpp` for the reranker as well. While [support for reranking is newer in `llama.cpp`](https://github.com/ggml-org/llama.cpp/issues/8555), there are a number of models that work, with decently small quantizations. I chose [`BAAI/bge-reranker-v2-m3`](https://huggingface.co/BAAI/bge-reranker-v2-m3) and used the 8-bit quantization.
...
- You might think that we could just select the 4 results not shown after reranking, but what if one of these is a very irrelevant false positive and the 9th result from vector search would have outperformed it?

![the problem with pagination](/assets/media/jfmm/pagination_problem.webp)

### Eliminating repeated results

`LIMIT` and `OFFSET` are the wrong tools for this job. Instead, it's better to keep a record of what values the user has seen and exclude them from the following queries. For example, suppose our first-page query goes like this:

- Vector search returns these IDs: `1`, `2`, `3`, `4`, `5`, `6`, `7`, and `8`.
- We rerank and select `2`, `4`, `6`, and `8` for our first page.

To select the vector search candidates for our second page, we can just exclude the exact results for our first page.
...
So where do we store this list? If we store it on the server or in the database, the server has to keep track of client sessions, which adds complexity. The easier option is probably to have the user's browser keep track of the list. But how?
...
- You need to use a framework like React, where previously HTML would have been enough.
...
- This approach will usually require two round-trips to the server: one to load the page and one to fetch the results. If network latency is your main bottleneck, this will double page load time.
- State is not persisted across reloads, so this will return the user to page 1 if the page is reloaded. To solve this, the developer will need to store the state somewhere, usually in the URL with a router framework like [React Router](https://reactrouter.com/), or in `localStorage`. Gross!
